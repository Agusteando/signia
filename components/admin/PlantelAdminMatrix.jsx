...
export default function PlantelAdminMatrix({ planteles, admins }) {
  ...
  return (
    <div id="plantel-admin-matrix" className="w-full bg-white border border-slate-200 shadow-sm rounded-xl p-4 sm:p-5 mb-6">
      ...
      <div className="mt-3 mb-5 w-full max-w-sm">
        <input
          ...
          type="text"
          className="w-full rounded-md px-3 py-1.5 sm:px-4 sm:py-2 border border-slate-300 shadow-sm placeholder-slate-400 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 transition-colors bg-slate-50 focus:bg-white"
        ...
        {planteles.map((p) => {
          ...
          return (
            <div key={p.id} className="border border-slate-200 rounded-xl overflow-hidden bg-white shadow-sm transition-shadow hover:shadow-md">
              <button
                className="flex flex-row items-center justify-between w-full px-4 py-3 sm:px-5 sm:py-4 font-semibold text-slate-800 text-sm rounded-xl focus:outline-none hover:bg-slate-50 transition"
              ...
              >
                ...
                <span className="flex items-center gap-2 sm:gap-3">
                  ...
                  {expanded === p.id
                    ? <ChevronUpIcon className="w-4 h-4 sm:w-5 sm:h-5 text-slate-400 ml-1 sm:ml-2" />
                    : <ChevronDownIcon className="w-4 h-4 sm:w-5 sm:h-5 text-slate-400 ml-1 sm:ml-2" />
                  }
                </span>
              </button>
              ...
              <div id={`plantel-expand-${p.id}`} className={expanded === p.id ? "block" : "hidden"}>
                <div className="p-0 border-t border-slate-100">
                  {loading[p.id] && <div className="p-6 sm:p-8 text-center text-slate-500 font-medium text-sm">Cargando datos del plantel...</div>}
...
function PlantelSummaryBar({ users = [], loading }) {
  if (loading === true || !Array.isArray(users)) {
    return <span className="text-[11px] sm:text-xs font-medium text-slate-400 ml-3 sm:ml-6 animate-pulse">Calculando...</span>;
  }
  ...
  return (
    <span className="flex flex-row gap-2 sm:gap-3 items-center ml-2 sm:ml-6">
      <span className="text-[11px] sm:text-xs text-slate-500 font-medium">{userTotal} reg.</span>
      ...
      <span className="text-[11px] sm:text-xs text-indigo-700 font-semibold">{userDoneCount}/{userTotal} validados</span>
    </span>
  );
}
        ...
        <tbody>
          ...
          {filteredUsers.map(u => (
            <tr key={u.id} className="border-b border-slate-100 hover:bg-white transition bg-slate-50">
              <td
                className="sticky left-0 bg-white z-20 border-r border-slate-200 flex flex-row gap-2.5 items-center px-3 py-2"
                style={{ minWidth: col1Width, maxWidth: col1Width, width: col1Width, boxShadow: "1px 0 0 #e2e8f0" }}
              >
                <img 
                  ...
                  height={28} 
                  alt="" 
                  className="rounded-full bg-slate-100 border border-slate-200 object-cover shrink-0" 
                  onError={(e) => { e.target.onerror = null; e.target.src = "/IMAGOTIPO-IECS-IEDIS.png"; }}
                />
                <div className="min-w-0 w-full">
                  <div className="font-semibold text-slate-900 truncate text-xs">{u.name}</div>
                  <div className="text-[10px] sm:text-[11px] text-slate-500 truncate">{u.email}</div>
                </div>
              </td>
              ...
              <td className="px-3 py-2">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] sm:text-[11px] font-medium text-slate-600">{u.userProgress.done}/{u.userProgress.total}</span>
                </div>
              </td>
              <td className="px-3 py-2 text-center">
                {u.adminProgress.proyectivosUploaded
                  ? <span className="inline-flex items-center gap-1 text-[10px] font-medium rounded-md bg-emerald-50 border border-emerald-200 px-1.5 py-0.5 text-emerald-700"><CheckCircleIcon className="w-3.5 h-3.5" />Entregados</span>
                  : <span className="inline-flex items-center gap-1 text-[10px] font-medium rounded-md bg-amber-50 border border-amber-200 px-1.5 py-0.5 text-amber-700"><ClockIcon className="w-3.5 h-3.5" />Falta</span>
                }
              </td>
              <td className="px-3 py-2 text-center">
                {u.fullyCompleted
                  ? <span className="inline-flex items-center gap-1 text-[10px] font-medium rounded-md bg-emerald-50 border border-emerald-200 px-1.5 py-0.5 text-emerald-700"><CheckCircleIcon className="w-3.5 h-3.5" />Completo</span>
                  : <span className="inline-flex items-center gap-1 text-[10px] font-medium rounded-md bg-slate-100 border border-slate-200 px-1.5 py-0.5 text-slate-600"><XCircleIcon className="w-3.5 h-3.5" />En proceso</span>}
              </td>
            </tr>
          ))}
        </tbody>
