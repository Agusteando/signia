...
export default function UserRow({
  ...
  return (
    <tr className={`border-b border-slate-100 hover:bg-slate-50 transition ${selected ? "bg-indigo-50/50 hover:bg-indigo-50" : ""} ${!isActive ? "opacity-60 bg-slate-50" : ""}`}>
      <td className="text-center px-2 py-2 sm:px-3 sm:py-3">
        <input
          type="checkbox"
          checked={selected}
          onChange={e => onSelect(e.target.checked)}
          className="accent-indigo-600 w-4 h-4 rounded border-slate-300 cursor-pointer"
          aria-label="Seleccionar usuario"
        />
      </td>
      <td className="px-2 py-2 sm:px-3 sm:py-3 flex flex-row gap-3 items-center min-w-[200px]">
        <img
          src={user.picture || "/IMAGOTIPO-IECS-IEDIS.png"}
          alt=""
          className="w-9 h-9 rounded-full object-cover bg-white border border-slate-200 shadow-sm shrink-0"
        ...
        {canAssignPlantel ? (
          ...
          <span className="text-xs sm:text-sm text-slate-700 font-medium">
            {planteles.find(p => String(p.id) === String(user.plantelId))?.name ||
              <span className="text-slate-400 italic font-normal text-xs">Sin plantel</span>}
          </span>
        )}
      </td>
      <td className="px-2 py-2 sm:px-3 sm:py-3">
      ...
      <td className="px-2 py-2 sm:px-3 sm:py-3 min-w-[130px] align-middle">
        <div className="flex items-center gap-2">
          <div className="flex-1 relative min-w-[60px] max-w-[100px]">
          ...
          <span className="text-[11px] sm:text-xs font-medium text-slate-600">{progress.done}/{progress.total}</span>
          {progress.pct === 100
            ? <span className="hidden md:flex ml-1 text-emerald-600 items-center gap-0.5 text-[10px] font-medium"><CheckCircleIcon className="w-3.5 h-3.5" />Listo</span>
            : null
          }
        </div>
      </td>
      ...
      <td className="px-2 py-2 sm:px-3 sm:py-3">
        <div className="flex flex-row flex-wrap gap-1.5 items-center min-w-[100px]">
          {canToggleActive && (
            isActive ? (
              <button
                title="Dar de baja"
                className="inline-flex flex-1 justify-center items-center gap-1 px-2 py-1.5 rounded-md bg-white hover:bg-slate-50 border border-slate-300 text-slate-700 text-[10px] sm:text-[11px] font-medium shadow-sm transition-shadow focus:outline-none"
                onClick={() => setConfirmAction({ type: "toggle", user })}
                aria-label="Dar de baja"
              >
                <PowerIcon className="w-3.5 h-3.5 text-slate-400" />
                Baja
              </button>
            ) : (
              <button
                title="Activar"
                className="inline-flex flex-1 justify-center items-center gap-1 px-2 py-1.5 rounded-md bg-emerald-600 hover:bg-emerald-700 text-white text-[10px] sm:text-[11px] font-medium shadow-sm transition-colors focus:outline-none"
                onClick={() => setConfirmAction({ type: "toggle", user })}
                aria-label="Activar"
              >
                <CheckCircleIcon className="w-3.5 h-3.5 text-emerald-100" />
                Activar
              </button>
            )
          )}
          {canDelete && (
            <button
              ...
              className="inline-flex flex-1 justify-center items-center gap-1 px-2 py-1.5 rounded-md bg-white border border-red-200 text-red-600 text-[10px] sm:text-[11px] font-medium shadow-sm transition-colors hover:bg-red-50 focus:outline-none opacity-60 hover:opacity-100"
